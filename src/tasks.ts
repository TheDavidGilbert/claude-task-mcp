...
import {
  Task, TaskComment, TaskLink, TaskWithDetails, ProjectStats, Project,
...
export function createTask(
  ...
  const createFn = db.transaction(() => {
    ...
    return db.prepare(`
      INSERT INTO tasks (project_id, task_number, task_id, title, description, type, priority, stage, estimate)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      RETURNING *
    `).get(
      projectId,
...
export function updateTask(
  db: Database.Database,
  taskId: string,
  updates: Partial<Pick<Task, 'title' | 'description' | 'type' | 'priority' | 'stage' | 'estimate'>>
): Task {
  const task = db.prepare('SELECT * FROM tasks WHERE task_id = ?').get(taskId) as Task | undefined;
  if (!task) throw new Error(`Task ${taskId} not found`);

  const allowed: Array<keyof typeof updates> = ['title', 'description', 'type', 'priority', 'stage', 'estimate'];
